...
class VacuumCleanerApp:
    ...
    def auto_move(self):
        ...
        nearest_dust = min(self.dust_positions, key=lambda pos: self.bfs(self.vacuum_position, pos))
        self.move_vacuum_to(nearest_dust)
        self.root.after(500, self.auto_move)

    def move_vacuum_to(self, target):
        """Moves the vacuum cleaner to the target position using BFS."""
        path = self.bfs(self.vacuum_position, target)
        if path:
    ...
    def display_summary(self):
        ...
        optimality = "Optimal"  # BFS guarantees the shortest path
        time_complexity = "O(n)"  # Time complexity for BFS, where n is the number of grid cells
        space_complexity = "O(n)"  # Space complexity for BFS
